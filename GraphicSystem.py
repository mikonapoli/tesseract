...
class BoardRenderer(sdl2.ext.Renderer):
    def __init__(self, board_surface, board_object, hidden_rows = 2, center = False):
    ...
    def render_board(self, piece):
        ...
        pos_y = 0
        if self.centered:
            if piece.piecedata.type == "o": w = 4
            else: w = bbox[1] + 1
            if piece.piecedata.type == "i": h = 1
            else: h = 2
        ...
        for y in range(self.hid,self.bsize[1]):
            for x in range(self.bsize[0]):
                if self.board.boardstatus.map[y][x] != 0:
                    ...
                    self.hstep,self.vstep), self.color_code[self.board.boardstatus.map[y][x]]))
