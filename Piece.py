...
class VirtualPieceChecker(sdl2.ext.Applicator):
    ...
    def _out_of_board(self,piece):
        ...
        piece.boardposition.x + piece.bbox[piece.rot][1] >= self.board_size[0] or
        piece.boardposition.y + piece.bbox[piece.rot][2] >= self.board_size[1]):
    ...
    def _board_collision(self, piece):
        ...
        for i in range(bbox[0],bbox[1]+1):
            for j in range(bbox[2]+1):
                ...
                self.board.boardstatus.map[piece.boardposition.y + j][piece.boardposition.x + i] != 0):
                    collision = True
        ...
        for vp, pd in componentsets:
            ...
            if pd.blocked and piece_rotated:
                ...
                for test in kickmap[ind]:
                    if pd.blocked:
                        vp.boardposition.x = pd.boardposition.x + (rotation_type * test[0])
                        vp.boardposition.y = pd.boardposition.y + (rotation_type * test[1])
                        pd.blocked = self._out_of_board(vp) or self._board_collision(vp)
...
class PieceMovement:
    ...
    def rotate(self,piece, mov = 1):
        old_index = piece.virtualpiece.rot
        new_index = (piece.virtualpiece.rot + mov)%len(piece.virtualpiece.rotmap)
...
class VirtualPiece(object):
    def __init__(self, rotation, rotation_map, color, piecetype, posx, posy, bounding_box):
...
class Piece(sdl2.ext.Entity):
    def __init__(self, world, piecetype, posx, posy, rotation, rotationmap, color, rotpos):
        
        super(Piece, self).__init__()
        
        self.piecedata = PieceData(rotation, rotationmap,color, piecetype, posx, posy, rotpos)
        self.virtualpiece = VirtualPiece(rotation, rotationmap,color, piecetype, posx, posy, rotpos)